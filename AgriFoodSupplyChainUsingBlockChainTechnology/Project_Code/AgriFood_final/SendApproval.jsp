				...
				<div class="row">
					<div class="col-md-12">
						<div class="col-lg-12">
							...
							<h2><b>Send Product For Approval</b></h2>
						<br><br>
							</div>
						</div>
                    ...
                    /*  ResultSet rs=st.executeQuery("select * from addproduct"); */
                     ResultSet rs=st.executeQuery("select addnewproduct.id,addnewproduct.hashcode,addnewproduct.image,addnewproduct.imagename,addnewproduct.name,addnewproduct.price,addnewproduct.weight,user.selectfield, user.firstname,user.lastname,user.mobileno, user.citySelect,addnewproduct.status from addnewproduct inner join user on addnewproduct.mobileno=user.mobileno");  
                     ...
                     if(rs.next() == false) {
                    	...
                    	<%-- <td><img alt="Profile Pic" class="img-circle" src="Profilepic.jsp?id=<%=rs.getInt(1) %>"  width="100px" height="80px"/></td> --%>
                    	 ...
                    	 <td style="color:black"><%=rs.getString(2) %></td>
                    	  <td><img alt="Profile Pic" class="img-circle" src="AddNewProductPic.jsp?id=<%=rs.getInt(1) %>"  width="100px" height="80px"/></td>                    	 
                    	...
                    	<td style="color:black"><%=rs.getString(9) %> <%=rs.getString(10) %></td>
                    	...
                    	<td style="color:black"><%=rs.getString(13) %></td>
                    	 ...
                    	 <%-- <th><a href="SendApprovalCon?id=<%=rs.getInt(1)%>&status=<%=rs.getString(13)%>"><%=rs.getString(13)%></a></th> 
                    	  --%>
                    	<%--  <td><a href="DeleteCon?id=<%=rs.getInt(1)%>" style="color:red"><b>Delete</b></a></td> --%>
                    	 </TR>                    	 
                    <%count++; }while(rs.next());}%> 
